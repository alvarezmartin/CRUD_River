...
export default function TarjetaDef({data,setDefensorEliminado}){

    const eliminarDefensor=async()=>{
        if(window.confirm(`¿Desea eliminar a ${data.nombre} ${data.apellido}?`)){
            let id=data._id
            let respuesta = await fetch("http://localhost:4000/eliminardefensor",{
                method:'delete',
                headers:{
            ...
            .then(()=>{setDefensorEliminado(true); setTimeout(()=>{setDefensorEliminado(false)},2000)})
            .then((setTimeout(()=>{window.location.reload()},2000)))
            .catch((err)=>console.log(err))
        } else{}
    }

    const editarDefensor=async(e)=>{
        let id=data._id
        const formInfo = new FormData(e.target)

        formInfo.append("imagen",e.target[2].value)

        let resultado = await fetch(`http://localhost:4000/editardefensor/${id}`,{
            method:"put",
    ...
    return(
        ...
        <div className="cuadro card d-flex justify-content-evenly">
            ...
            <a type='button'><i className="fa-solid fa-trash" title="Eliminar jugador" onClick={eliminarDefensor}></i></a>
        ...
        {modal && (
            <div className='overlay'>
                <div className="modal2" id="editarDefensor">
                    <h2>Editar a {data.nombre} {data.apellido}</h2>
                    <form onSubmit={editarDefensor}>
